...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        ...
        resnets = [
            ResnetBlock2D(
                in_channels=in_channels,
                out_channels=in_channels, #min블럭이라 input채널과 output채널이 같은형태
                temb_channels=temb_channels,
                eps=resnet_eps,
                groups=resnet_groups,
                dropout=dropout,
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = resnet(hidden_states, temb, scale=lora_scale) #hidden_states = 옷이이미지를 인코딩한거 이거 resnet한번 돌리고 그 이후에 transformer이용해서 특징추출
                hidden_states,out_garment_feat = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
        ...
        return hidden_states, output_states,garment_features #output_states는 hiddens_states상태들 모아놓은 튜플형태.
